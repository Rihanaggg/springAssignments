...
import com.prodapt.learningspring.controller.binding.UserService;
...
public class LoginController {
    ...
    @PostMapping
    public String login(@ModelAttribute("user") Login user, BindingResult bindingResult, RedirectAttributes attr) {
        if (bindingResult.hasErrors()) {
            attr.addFlashAttribute("org.springframework.validation.BindingResult.user", bindingResult);
            attr.addFlashAttribute("user", user);
            return "redirect:/loginpage";
        }
        Optional<Login> authResult = userService.authenticate(user.getName(), user.getPassword());
